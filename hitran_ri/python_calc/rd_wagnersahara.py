#! /usr/bin/env python
"""rd_wagnersahara.py -  specify the refractive indices"""
def rd_wagnersahara():
    import os
    import math
    import numpy as np
...
# Will select the jset compund to work with
    listset=['0 Bakina ','1 Cairo ','2 SANUM1 ','3 SANUM2 ','4 SANUM3 ']
...
# write out all possible compounds
    ...
    if iwrsp == 1:
        file_object.write("\n")
        file_object.write("\n")
        file_object.write(" rd_wagnersahara: listset ")
        for i in range(0,5):
            file_object.write("\n")
            a0=listset[i]
            listn1=[a0]
...
# Write out to console
    iwrsp=2
    if iwrsp == 2:
        print(" rd_wagnersahara: listset ")
        for i in range(0,5):
            a0=listset[i]
            listn1=[a0]
            print(str(listn1))
        print(" enter jset from 0 to 4")

# Will specify jset e.g. jset=3
    jsetstr=input(" jset: ")
    jset=int(jsetstr)

    titlegr="Wagner Dust "+listset[jset] 

# *****************
# Will read in from the wagnersahara.dat ascii file
...
# Write out results
    if nopr == 1:
        file_object.write("\n")
        file_object.write("\n")
        file_object.write(" rd_wagnersahara: fileasciidir ")
        file_object.write(str(fileasciidir))
        file_object.write("\n")
        file_object.write(" rd_wagnersahara: fdat ")
        file_object.write(str(fdat))
        file_object.write("\n")
        file_object.write(" rd_wagnersahara: filework ")
        file_object.write(str(filework))

        file_object.write("\n")
        file_object.write("\n")
        file_object.write(" rd_wagnersahara: i,wavedat,wcmdat,rndex,ridex ")
        for i in range(0,ndat):
            file_object.write("\n")
            a0="% 4d" %i
            a1="% 11.4f" %wavedat[i]
...
#   print("list5 from rd_wagnersahara.py ",list5)
