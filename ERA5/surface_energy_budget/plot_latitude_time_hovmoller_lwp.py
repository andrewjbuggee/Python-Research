#!/usr/bin/env python3
"""Latitude-time (Hovmoller) section of cloud liquid water path.
...
other respect to ``plot_latitude_time_hovmoller.py``, whose season-window,
year-averaging, strip-averaging and sea ice contour machinery this script
reuses (imported, not copied, so the two cannot drift apart). See that
script's docstring for those conventions. This one plots a single quantity,
LWP (``tclw``), rather than offering a choice of longwave fields.

Median, not mean
-----------------
Matching ``plot_monthly_lwp_maps.py``: LWP in cloudy Arctic scenes is heavily
right-skewed and, in winter, piled up near zero, so the median -- not the mean
-- is taken at both reduction steps (across the day-of-season slot, and across
seasons). Only LWP is reduced this way; sea ice stays a mean, as in the maps
script.

Threshold
---------
ERA5 carries trace liquid almost everywhere (see ``analyze_cloud_liquid_
frequency.py``), so an unthresholded median is rarely a clean zero even in
midwinter. ``--lwp-threshold`` (default 0.03 g m-2, matching ``plot_monthly_
lwp_maps.py``) zeroes any hourly LWP at or below it BEFORE the strip average
(a mean, over the 3-cell longitude strip only) and the two median reductions
are taken, so a period whose typical hour carries only numerical-floor liquid
reads as 0, not as a small positive value with no physical meaning.

Colour scale
------------
The same white-at-zero-to-dark-navy sequential ramp as ``plot_monthly_lwp_
maps.py`` (imported from there, again so the two cannot drift apart), with the
lower limit pinned to 0 g m-2 and the upper limit a robust percentile.

Examples
--------
Climatology, every complete season in the record::

    python plot_latitude_time_hovmoller_lwp.py --storage external --region barrow

One season::

    python plot_latitude_time_hovmoller_lwp.py --region barrow --years 2019

Several seasons, a stricter liquid threshold::

    python plot_latitude_time_hovmoller_lwp.py --region barrow \\
        --years 2019-2025 --lwp-threshold 0.1
"""
...
from plot_monthly_lwp_maps import (
    DEFAULT_LWP_THRESHOLD_G,
    LWP_CMAP,
    LWP_LABEL,
    LWP_UNITS,
    LWP_VAR,
...
def build_section(
    ...
    Returns a dict with the LWP field (g m-2, MEDIAN over each day-of-season
    slot), the sea ice field (mean), the latitude axis, the day-of-season
    labels, and the seasons found. LWP is thresholded (values at or below
    ``lwp_threshold_g`` zeroed) BEFORE the strip average across longitude, so
    trace liquid at ERA5's numerical floor does not leak into an otherwise-dry
    cell through averaging.
    """
    ...
    # to obtain a 2-variable, 3-column strip.
    band = (
        ds[[LWP_VAR, "siconc"]]
    ...
    lwp_raw = band[LWP_VAR].values * 1000.0            # kg m-2 -> g m-2
    lwp_thr = np.where(lwp_raw > lwp_threshold_g, lwp_raw, 0.0)
    lwp_thr[np.isnan(band[LWP_VAR].values)] = np.nan    # keep genuine gaps as NaN
    ...
    field_t = nanmean_quiet(lwp_thr, axis=2)               # (time, lat)
    ...
    for s_i, s in enumerate(uniq_seasons):
        ...
        for slot in np.unique(dos[sel_s]):
            sel = sel_s & (dos == slot)
            stack[s_i, slot] = nanmedian_quiet(field_t[sel], axis=0)
...
def make_hovmoller(
    ...
    field = nanmedian_quiet(sec["field"], axis=0)   # median over seasons
    ...
    # Lower limit pinned to 0, not a robust-low percentile: 0 is "no liquid", a
    # meaningful floor for LWP, and letting the low end float above it would
    # make some genuinely dry columns look non-white.
    finite = field[np.isfinite(field)]
    vmin = 0.0
    vmax = float(np.percentile(finite, COLOR_PERCENTILE)) if finite.size else 1.0
    if vmax <= vmin:
        vmax = vmin + 1.0

    fig, ax = plt.subplots(figsize=(13, 5.6), constrained_layout=True)
    mesh = ax.pcolormesh(x, lat, field.T, cmap=LWP_CMAP, vmin=vmin, vmax=vmax,
                         shading="auto")
    ...
    if np.isfinite(ice).any():
        ...
        if contour_style == "contrast":
            # A plain black line disappears against the dark-navy end of the
    ...
    ax.set_title(
        f"{LWP_LABEL} — {region}, meridional strip north of Utqiaġvik\n"
        f"{mode_label}   |   {lon_used.size}-cell longitude mean "
        f"({lon_used.min():.2f} to {lon_used.max():.2f}°E)   |   "
        f"threshold {lwp_threshold_g:g} g m$^{{-2}}$   |   "
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--lwp-threshold", type=float,
                        ...
                        help="LWP in g m-2 at or below which an hour is treated as "
                             ...
                             "near-zero trace liquid making a dry period look "
                             "falsely non-zero.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72)
    print("Latitude-time section of cloud liquid water path")
    print("=" * 72)
    ...
    mode_label = format_used_seasons(used, stat="median")
    ...
    median_field = nanmedian_quiet(sec["field"], axis=0)
    finite = median_field[np.isfinite(median_field)]
    print(f"\n  {LWP_LABEL}: median={np.median(finite):.3f}, "
          f"range {finite.min():.3f} to {finite.max():.3f} [g m-2]")

    tag = f"season{used[0]}" if len(used) == 1 else f"median{used[0]}-{used[-1]}"
    make_hovmoller(sec, args.region, mode_label, args.lwp_threshold,
                   tuple(args.ice_levels), contour_style=args.contour_style,
                   output_path=out_dir / f"{args.region}_hovmoller_lwp_{tag}.png",
                   dpi=args.dpi)
