...
The 7 retrieval levels are evenly spaced in altitude between cloud top and
base, so the level spacing cancels in the integral ratio — no z grid needed.
...
# Figures
# ─────────────────────────────────────────────────────────────────────────────
def plot_lwp_diagnostics(lwp_true, lwp_ret, err, out_path: Path) -> None:
    """2×2 IEEE-style diagnostics for the LWP retrieval."""
    setup_style_paper()
    ...
    ax.set_title(fr'(c)  Pearson $r$ = {r:+.3f}'  '\n'
                 fr'      Spearman $\rho$ = {rho:+.3f}', fontsize=8.5)
    ax.grid(alpha=0.3, which='both')

    # (d) relative-error histogram
    ax = axes[1, 1]
    ...
    ax.set_ylabel('count')
    ax.set_title(fr'(d)  median = {med:+.1f} %'  '\n'
...
def main():
    ...
    plot_lwp_diagnostics(lwp_true, lwp_ret, err, out1)
